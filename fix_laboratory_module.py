...
# -*- coding: utf-8 -*-
"""
Script pour corriger le département de laboratoire
"""
import os

def fix_laboratory_module():
    lab_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "app", "ui", "departments", "laboratory.py")
    dashboard_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "app", "ui", "dashboard.py")
    
    # 1. D'abord, vérifions le fichier dashboard.py pour voir comment il initialise LaboratoryFrame
    with open(dashboard_path, 'r') as f:
        dashboard_content = f.read()
    
    # Trouver la partie qui initialise LaboratoryFrame
    lab_init_pos = dashboard_content.find("self.dept_frame = LaboratoryFrame(")
    if lab_init_pos > 0:
        # Extraire la ligne
        line_end = dashboard_content.find(")", lab_init_pos)
        lab_init_line = dashboard_content[lab_init_pos:line_end+1]
        print(f"Ligne d'initialisation du laboratoire: {lab_init_line}")
        
    # 2. Maintenant, corrigeons laboratory.py
    laboratory_content = """\"\"\"
...
"""
    ...
    with open(lab_path, 'w') as f:
        f.write(laboratory_content)
    
    print(f"✅ Module laboratory.py corrigé")

if __name__ == "__main__":
    print("Correction du module de laboratoire...")
